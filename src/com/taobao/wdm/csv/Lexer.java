...
package com.taobao.wdm.csv;

import static com.taobao.wdm.csv.Constants.BACKSPACE;
import static com.taobao.wdm.csv.Constants.CR;
import static com.taobao.wdm.csv.Constants.END_OF_STREAM;
import static com.taobao.wdm.csv.Constants.FF;
import static com.taobao.wdm.csv.Constants.LF;
import static com.taobao.wdm.csv.Constants.TAB;
import static com.taobao.wdm.csv.Constants.UNDEFINED;
import static com.taobao.wdm.csv.Token.Type.COMMENT;
import static com.taobao.wdm.csv.Token.Type.EOF;
import static com.taobao.wdm.csv.Token.Type.EORECORD;
import static com.taobao.wdm.csv.Token.Type.INVALID;
import static com.taobao.wdm.csv.Token.Type.TOKEN;
...
final class Lexer {
	 ...
	 * (-1), and because the Unicode value {@code FFFE} would be encoded as two
	 * chars (using surrogates) and thus there should never be a collision with
	 * a real text char.
	 */
	...
	/**
	 * INTERNAL API. but ctor needs to be called dynamically by PerformanceTest
	 * class
	 */
	...
	Token nextToken(final Token token) throws IOException {
		 ...
		 * need to know if the last char was CR or LF - they are equivalent
		 * here.
		 */
	 ...
	 *             on stream access error
	 */
	private Token parseSimpleToken(final Token token, int ch)
			throws IOException {
		...
		while (true) {
			if (readEndOfLine(ch)) {
				...
				if (unescaped == Constants.END_OF_STREAM) { // unexpected char
															// after escape
					token.content.append((char) ch).append(
							(char) in.getLastChar());
				} else {
	 ...
	 * and after an encapsulated token are ignored. The token is finished when
	 * one of the following conditions become true:
	 * <ul>
	 ...
	 * @throws IOException
	 *             on invalid state: EOF before closing encapsulator or invalid
	 *             character before delimiter or EOL
	 */
	private Token parseEncapsulatedToken(final Token token) throws IOException {
		...
		while (true) {
			...
			if (isEscape(c)) {
				...
				if (unescaped == Constants.END_OF_STREAM) { // unexpected char
															// after escape
					token.content.append((char) c).append(
							(char) in.getLastChar());
				} else {
					token.content.append((char) unescaped);
				}
	 ...
	 *         invalid.
	 * @throws IOException
	 *             if there is a problem reading the stream or the end of stream
	 *             is detected: the escape character is not allowed at end of
	 *             strem
	 */
	...
	/**
	 * Checks if the current character represents the start of a line: a CR, LF
